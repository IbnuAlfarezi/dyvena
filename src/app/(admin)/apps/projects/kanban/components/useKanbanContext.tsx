...
export const kanbanTaskSchema = yup.object({
  ...
  assignee: yup.mixed<KanbanTaskType['assignee']>().required('Please select an assignee'),
  ...
  const handleNewTask = newTaskHandleSubmit((values: TaskFormFields) => {
    ...
    if (activeSectionId) {
      const newTask: KanbanTaskType = {
        ...
        assignee: { image: formData.assignee.image ?? user1, name: formData.assignee.name },
        stats: { task: 5, time: '1 days', link: 3, messages: 8 },
