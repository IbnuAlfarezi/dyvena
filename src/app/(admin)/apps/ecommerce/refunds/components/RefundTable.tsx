...
const RefundTable = () => {
  const columns: ColumnDef<RefundType, any>[] = [
    ...
    columnHelper.accessor('product', {
      ...
      cell: ({ row }) => (
        <div className="d-flex align-items-center gap-2">
          <Image src={row.original.product.image} alt="" className="rounded" width="34" height="34" />
          <div>
      ...
      cell: ({ row }) => (
        <div className="d-flex justify-content-start align-items-center gap-2">
          <div className="avatar avatar-sm">
            <Image src={row.original.customer.image} alt="avatar-5" className="img-fluid rounded-circle" />
          </div>
          <div>
    ...
    columnHelper.accessor('payment', {
      ...
      cell: ({ row }) => (
        <>
          <Image src={row.original.payment.image} alt="" className="me-2" height="28" /> {row.original.payment.type === 'card' ? row.original.payment.number : row.original.payment.name}
        </>
      ),
    }),
