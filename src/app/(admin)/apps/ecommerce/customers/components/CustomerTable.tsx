...
const CustomerTable = () => {
  const columns = [
      ...
      cell: ({ row }) => (
        <div className="d-flex align-items-center gap-2">
          <div className="avatar avatar-sm">
            <Image src={row.original.customer.image} alt="" height={32} width={32} className="img-fluid rounded-circle" />
          </div>
          <div>
            <h5 className="mb-0">
    ...
    columnHelper.accessor('country', {
      ...
      cell: ({ row }) => (
        <>
          <Image src={row.original.country.flag} alt={row.original.country.name} className="rounded-circle me-1" height={16} width={16} /> {row.original.country.name}
        </>
      ),
    }),
    ...
    }),
    {
      ...
      cell: ({ row }: { row: TableRow<CustomerType> }) => (
        <div className="d-flex gap-1">
          <Button variant="default" size="sm" className="btn-icon btn-sm ">
